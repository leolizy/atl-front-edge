  ...
  forbidden: [
    {
      ...
      severity: "error",
      // Importer is NOT inside any package. (`^${R}/[^/]+/` rather than `^${R}/`
      // so loose root files like src/index.ts are covered too — they live at
      // the packages root, outside every package.)
      from: { pathNot: `^${R}/[^/]+/` },
