Packages are deep modules — see [src/README.md](./src/README.md) before adding or importing one.

## Agent skills

### Issue tracker

Issues live as GitHub issues in `leolizy/narwhal-data-hub` (use the `gh` CLI); the local board under `.scratch/` is a read-only archive. See `docs/agents/issue-tracker.md`.
